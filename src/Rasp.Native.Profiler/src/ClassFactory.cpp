...
// Copyright (c) .NET Foundation and contributors). Unchanged except for CreateInstance
// instantiating RaspProfiler instead of the sample's CorProfiler.
...
HRESULT STDMETHODCALLTYPE ClassFactory::CreateInstance(IUnknown *pUnkOuter, REFIID riid, void **ppvObject)
{
    ...
    RaspProfiler* profiler = new RaspProfiler();
    if (profiler == nullptr)
    {
        return E_FAIL;
    }
