...
public class RaspGrpcGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // 2. GERAÇÃO DOS INTERCEPTORS INDIVIDUAIS
        ...
        // 3. GERAÇÃO DO AUTO-INSTALLER (DI)
    ...
    private static void GenerateInterceptor(SourceProductionContext context, ServiceMetadata service)
    {
        ...
        foreach (var method in service.Methods)
        {
            ...
            sb.AppendLine($"    private void Validate_{method.MethodName}({method.RequestType} req, string contextName)");
            sb.AppendLine("    {");
            
            foreach (var prop in method.Properties)
            {
                var pathSafe = prop.Path.Replace(".", "?.");
                ...
                {
                    // FIX CRÍTICO AQUI: Variável 'varName' e 'span' correta para escalares
                    var varName = $"val_{Math.Abs(prop.Path.GetHashCode())}";
                    sb.AppendLine($$"""
                        var {{varName}} = req.{{pathSafe}};
                        if (!string.IsNullOrEmpty({{varName}}))
                        {
                            var span = {{varName}}.AsSpan();

                            // XSS Check
                            var xssRes = _xssEngine.Inspect(span, "{{prop.Path}}");
                            if (xssRes.IsThreat)
                            {
                                _bus.PushAlert(xssRes.ThreatType ?? "Unknown", {{varName}}, $"{{prop.Path}} in {contextName}");
                                throw new RpcException(new Status(StatusCode.InvalidArgument, "RASP Security Violation"));
                            }
                            ...
                            if (sqlRes.IsThreat)
                            {
                                _bus.PushAlert(sqlRes.ThreatType ?? "Unknown", {{varName}}, $"{{prop.Path}} in {contextName}");
                                throw new RpcException(new Status(StatusCode.InvalidArgument, "RASP Security Violation"));
                            }
                        }
                    """);
    ...
    private static void GenerateInstaller(SourceProductionContext context, ImmutableArray<ServiceMetadata?> services)
    {
        ...
        sb.AppendLine("""
            ...
            namespace Rasp.Instrumentation;

            public static class RaspServiceCollectionExtensions
            {
                /// <summary>
                /// Automatically registers all RASP Interceptors found in the assembly.
                /// </summary>
                public static IGrpcServerBuilder AddRaspSecurity(this IGrpcServerBuilder builder)
                {
            """);

        // 1. Registrar os Interceptors no DI (Necessário para Injeção de Dependência)
        foreach (var service in services)
        {
            if (service is null) continue;
            sb.AppendLine($"        builder.Services.AddSingleton<{service.Namespace}.{service.ServiceName}RaspInterceptor>();");
        }

        // 2. Configurar o GrpcServiceOptions (A forma canônica de adicionar interceptors globais)
        sb.AppendLine();
        ...
        {
            if (service is null) continue;
            sb.AppendLine($"            options.Interceptors.Add<{service.Namespace}.{service.ServiceName}RaspInterceptor>();");
        }
